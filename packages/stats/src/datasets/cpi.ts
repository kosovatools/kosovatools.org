...
function buildCpiGroups(dataset: RawCpiDataset): CpiGroupSeries[] {
  return Array.isArray(dataset?.groups)
    ? dataset.groups.map(normalizeGroup)
    : [];
}

export const cpiChangeMonthlyGroups = buildCpiGroups(
  cpiChange as RawCpiDataset,
);

export const cpiIndexMonthlyGroups = buildCpiGroups(cpiIndex as RawCpiDataset);
