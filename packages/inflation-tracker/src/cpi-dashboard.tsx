...
export function InflationDashboard({
  ...
  const handleMetricChange = React.useCallback((next: Metric) => {
    setMetric(next);
  }, []);

  const handleRangeChange = React.useCallback((next: RangeOption) => {
    setRange(next);
  }, []);

  const handleGroupingChange = React.useCallback((next: PeriodGrouping) => {
    setPeriodGrouping(next);
  }, []);
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(260px,320px)_minmax(0,1fr)] lg:items-start">
            ...
            <div className="flex flex-col gap-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <Label>Grupimi i periudhave</Label>
                  <div className="flex flex-wrap gap-2">
                    {PERIOD_GROUPING_OPTIONS.map((option) => {
                      ...
                      return (
                        <Button
                          ...
                          size="sm"
                          variant={active ? "default" : "outline"}
                          onClick={() => handleGroupingChange(option.id)}
                        >
                          {option.label}
                        </Button>
                      );
